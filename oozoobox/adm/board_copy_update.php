...
sql_query(" ALTER TABLE `{$g5['write_prefix']}{$target_table}` ADD `as_level` int(11) NOT NULL DEFAULT '1' AFTER `as_extra` ", false);
...
sql_query(" ALTER TABLE `{$g5['write_prefix']}{$target_table}` ADD `as_icon` varchar(255) NOT NULL AFTER `as_tag` ", false);
sql_query(" ALTER TABLE `{$g5['write_prefix']}{$target_table}` ADD `as_update` datetime NOT NULL default '0000-00-00 00:00:00' AFTER `as_icon` ", false);
