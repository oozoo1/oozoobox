...
if($mode == "pform") {
	...
	if(isset($_POST['pt_del']) && $_POST['pt_del']) {
		...
		//회원정보에서 정보변경
		sql_query(" update {$g5['member_table']} set as_partner = '0' where mb_id = '$pt_id' ", false);
	...
	if($_POST['pt_register']) {
		//승인시 회원정보변경
		sql_query(" update {$g5['member_table']} set as_partner = '1' where mb_id = '$pt_id' ", false);
	}
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        <th scope="row">아이디</th>
        <td><?php echo get_sideview($mb['pt_id'], $mb['pt_id'], $mb['pt_email'], '');?></td>
        <th scope="row">신청일</th>
...
this.form.pt_leave.value=this.value; } else { this.form.pt_leave.value=this.form.pt_leave.defaultValue; }">
	...
	<td colspan="4" style="padding-left:0px;padding-top:30px;">
		<h2 class="h2_frm" style="margin:0px;padding:0px;">
			정산정보 
			<span class="apms-helper"> - 파트너별 수수료 미등록시 '기본설정'의 기본 수수료가 자동적용됩니다.</span>
		</h2>
	</td>
	</tr>
	<tr>
        <th scope="row">일반상품 수수료</th>
        <td>판매금액의 <input type="text" name="pt_commission_1" value="<?php echo $mb['pt_commission_1'] ?>" class="numeric sit_camt frm_input" size="4"> %</td>
        <th scope="row">일반상품 인센티브</th>
        <td>정산금액의 <input type="text" name="pt_incentive_1" value="<?php echo $mb['pt_incentive_1'] ?>" class="numeric sit_camt frm_input" size="4"> %</td>
    </tr>
	<tr>
        <th scope="row">컨텐츠상품 수수료</th>
        <td>판매금액의 <input type="text" name="pt_commission_2" value="<?php echo $mb['pt_commission_2'] ?>" class="numeric sit_camt frm_input" size="4"> %</td>
        <th scope="row">컨텐츠상품 인센티브</th>
        <td>정산금액의 <input type="text" name="pt_incentive_2" value="<?php echo $mb['pt_incentive_2'] ?>" class="numeric sit_camt frm_input" size="4"> %</td>
    </tr>
	<tr>
        <th scope="row">상품 등록비</th>
        <td><input type="text" name="pt_point" value="<?php echo $mb['pt_point'] ?>" class="frm_input sit_camt" size="13"> 점 (분류 등록비와 합산)</td>
        <th scope="row">일일 등록제한</th>
        <td><input type="text" name="pt_limit" value="<?php echo $mb['pt_limit'] ?>" class="frm_input sit_camt" size="4"> 개 상품 등록 가능</td>
	</tr>
	<tr>
