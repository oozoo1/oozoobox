...
if ($w == '') {
	...
	sql_query(" ALTER TABLE `{$create_table}` ADD `as_level` int(11) NOT NULL DEFAULT '1' AFTER `as_extra` ", false);
	...
	sql_query(" ALTER TABLE `{$create_table}` ADD `as_re_mb` varchar(20) NOT NULL DEFAULT '' AFTER `as_choice_cnt` ", false);
	...
	sql_query(" ALTER TABLE `{$create_table}` ADD `as_icon` varchar(255) NOT NULL AFTER `as_tag` ", false);
	sql_query(" ALTER TABLE `{$create_table}` ADD `as_update` datetime NOT NULL default '0000-00-00 00:00:00' AFTER `as_icon` ", false);
...
if (is_checked('chk_grp_as_comment_rows'))      $grp_fields .= " , as_comment_rows = '{$as_comment_rows}' ";
if (is_checked('chk_grp_as_comment_mobile_rows')) $grp_fields .= " , as_comment_mobile_rows = '{$as_comment_mobile_rows}' ";
...
if (is_checked('chk_all_as_comment_rows'))      $all_fields .= " , as_comment_rows = '{$as_comment_rows}' ";
if (is_checked('chk_all_as_comment_mobile_rows')) $all_fields .= " , as_comment_mobile_rows = '{$as_comment_mobile_rows}' ";
