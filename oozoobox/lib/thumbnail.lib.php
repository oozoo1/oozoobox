...
function get_view_thumbnail($contents, $thumb_width=0)
{
    ...
    for($i=0; $i<count($matches[1]); $i++) {
        ...
        if(is_file($srcfile)) {
            ...
            if ($width) {
                $thumb_tag = '<img src="'.G5_URL.str_replace($filename, $thumb_file, $data_path).'" alt="'.$alt.'" width="'.$width.'" height="'.$height.'"'.$align.' class="img-tag'.$class.'"/>';
            } else {
                $thumb_tag = '<img src="'.G5_URL.str_replace($filename, $thumb_file, $data_path).'" alt="'.$alt.'"'.$align.' class="img-tag'.$class.'"/>';
            }

            // $img_tag에 editor 경로가 있으면 원본보기 링크 추가
            if(!$link && (strpos($img_tag, G5_DATA_DIR.'/'.G5_EDITOR_DIR) || strpos($img_tag, G5_DATA_DIR.'/file')) && preg_match("/\.({$config['cf_image_extension']})$/i", $filename)) {
				if($is_lightbox) {
					$caption = ($alt) ? ' data-title="'.$alt.'"' : '';
	                $thumb_tag = '<a href="'.$src.'" data-lightbox="view-lightbox"'.$caption.' target="_blank">'.$thumb_tag.'</a>';
				} else {
