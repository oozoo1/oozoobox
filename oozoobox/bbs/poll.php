...
if($member['mb_level'] < $po['po_level']) {
	...
	alert($xp[$mg].'('.$member['mb_level'].') 이상 회원만 참여할 수 있습니다.');
}
...
if (!($search_ip || $search_mb_id)) {
    ...
    if ($is_member) { // 회원일 때는 id만 추가
        $mb_ids .= $member['mb_id'].',';
        $sql = " update {$g5['poll_table']} set po_score = po_score + {$ans}, po_cnt = po_cnt + 1, po_cnt{$ans} = po_cnt{$ans} + 1, mb_ids = '$mb_ids' where po_id = '$po_id' ";
    } else {
        $sql = " update {$g5['poll_table']} set po_score = po_score + {$ans}, po_cnt = po_cnt + 1, po_cnt{$ans} = po_cnt{$ans} + 1, po_ips = '$po_ips' where po_id = '$po_id' ";
    }
	...
	if($po['po_type'] == "1") { //별점
		...
		sql_query(" update {$write_table} set as_star_score = as_star_score + {$ans}, as_star_cnt = as_star_cnt + 1 where wr_id = '{$wr_id}' ", false);
		...
		sql_query(" update {$write_table} set as_poll = as_poll + 1 where wr_id = '{$wr_id}' ", false);
		sql_query(" update {$g5['board_new_table']} set as_poll + 1 where bo_table = '{$bo_table}' and wr_id = '{$wr_id}' ", false);
