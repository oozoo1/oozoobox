...
// 에디터 별도설정
if(isset($boset['editor']) && $boset['editor']) {
	$config['cf_editor'] = $boset['editor'];
...
if ($w == "u") {
    for ($i=0; $i<$file['count']; $i++) {
        if ($file[$i]['source']) {
			...
			if ($is_file_content) {
				$file_script .= "<div class='col-sm-5'><div class='form-group'><input type='text'name='bf_content[$i]' value='".addslashes(get_text($row['bf_content']))."' class='form-control input-sm' placeholder='이미지에 대한 내용을 입력하세요.'></div></div>";
			}
