...
if ($board['bo_use_category']) {
    ...
    for ($i=0; $i<count($categories); $i++) {
        ...
        if ($category==$sca) { // 현재 선택된 카테고리라면
            $category_option .= ' id="bo_cate_on"';
            $category_msg = '<span class="sound_only">浏览 </span>';
        }
...
if ($sca || $stx) {
    ...
    */
    ...
    $sql_search = "";
    $total_count = $board['bo_count_write'] - $bo_notice_cnt;
}
...
if (!$sst) {
    ...
    $sst = preg_match("/^(wr_datetime|wr_hit|wr_good|wr_nogood|as_view|as_down|as_download|as_update|wr_link1_hit|wr_link2_hit)$/i", $sst) ? $sst : "";
}
...
if ($sca || $stx) {
    ...
    $sql = " select * from {$write_table} where wr_is_comment = 0 {$sql_apms_where} {$sql_order} limit {$from_record}, $page_rows ";
}
