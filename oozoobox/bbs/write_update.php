...
if ($w == '' || $w == 'r') {
    ...
    // 새글 INSERT
    if($is_new) sql_query(" insert into {$g5['board_new_table']} ( bo_table, wr_id, wr_parent, bn_datetime, mb_id, as_re_mb ) values ( '{$bo_table}', '{$wr_id}', '{$wr_id}', '".G5_TIME_YMDHIS."', '{$member['mb_id']}', '{$as_re_mb}' ) ");
...
// 글업데이트
sql_query(" update {$write_table} set as_list = '{$as_list}' where wr_id = '{$wr_id}' ", false);
