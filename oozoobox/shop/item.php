...
// 이전 상품보기
$sql = " select it_id, it_name from {$g5['g5_shop_item_table']} where it_id > '$it_id' and (ca_id = '{$ca_id}' or ca_id2 = '{$ca_id}' or ca_id3 = '{$ca_id}') and it_use = '1' order by it_id asc limit 1 ";
$row = sql_fetch($sql);
if ($row['it_id']) {
    $prev_item = get_text($row['it_name']);
    $prev_href = './item.php?it_id='.$row['it_id'].'&amp;ca_id='.$ca_id;
} else {
    $prev_item = '';
    $prev_href = '';
}
...
// 다음 상품보기
$sql = " select it_id, it_name from {$g5['g5_shop_item_table']} where it_id < '$it_id' and (ca_id = '{$ca_id}' or ca_id2 = '{$ca_id}' or ca_id3 = '{$ca_id}') and it_use = '1' order by it_id desc limit 1 ";
$row = sql_fetch($sql);
if ($row['it_id']) {
    $next_item = get_text($row['it_name']);
    $next_href = './item.php?it_id='.$row['it_id'].'&amp;ca_id='.$ca_id;
} else {
    $next_item = '';
    $next_href = '';
}
...
for ($i=1; $i<=$len; $i++) {
	...
	$nav[$n]['ca_id'] = $code;
	$nav[$n]['on'] = ($ca_id === $code) ? true : false; // 현재 분류와 일치체크
